    ...
    return (
        <View style={{paddingRight: 10, paddingLeft: 10}}>
            <Tile
                ...
                titleStyle={{ color:colors.text, fontSize: 15, position: 'absolute', top: 3 }}
                containerStyle={{borderRadius: 8, backgroundColor: 'rgba(0,0,0,0)'}}
                activeOpacity={0.7}
                width={width / 2.5}
                height={height / 3}
                key={item.id}
                onPress={() => {navigation.push((routeName.name === 'SearchPage') ? 'InfoSearch' : 'InfoHome', {screen: 'Info', params: {id:item.id, title:{romaji: item.title.romaji, native: item.title.native}},});}}
                />
